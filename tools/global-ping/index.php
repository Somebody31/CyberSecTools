...
echo '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Global Ping Test</title>';
...
echo '<h1>Global Ping Test</h1>';
echo '<div class="desc">Measure network latency from multiple international locations to your server. Valuable for evaluating global performance, troubleshooting regional access issues, and CDN optimization.</div>';
echo '<form class="search-box" method="get">';
echo '<input type="text" name="host" placeholder="Enter domain or IP (e.g. example.com)" value="' . (isset($_GET['host']) ? htmlspecialchars($_GET['host']) : '') . '" required />';
echo '<button type="submit">Ping</button>';
echo '</form>';
if (isset($_GET['host']) && trim($_GET['host']) !== '') {
    ...
    echo '</div>';
    // Use backend logic
    $results = get_global_ping($host);
    $count = is_array($results) ? count($results) : 0;
    echo '<div class="results-header" style="margin-top:0;">Ping results for <b>' . htmlspecialchars($host) . '</b></div>';
    echo '<div class="results-count">Found ' . $count . ' locations.</div>';
    if ($tab === 'json') {
        $json = [
            "query" => ["tool" => "global-ping", "host" => $host],
            "response" => [
                "location_count" => strval($count),
                "results" => $results
            ]
        ];
        ...
        echo '<table style="margin-top:0;"><tr><th>Location</th><th>IP</th><th>Latency (ms)</th></tr>';
        if (is_array($results)) {
            foreach ($results as $row) {
                echo '<tr><td>' . htmlspecialchars($row['location']) . '</td><td>' . htmlspecialchars($row['ip']) . '</td><td>' . htmlspecialchars($row['latency']) . '</td></tr>';
            }
        }
        echo '</table>';
    }
