<?php
// index.php - DNS Propagation Checker (PHP only, no external CSS/JS/HTML)
// UI inspired by reverse-mx-lookup/index.php and the provided screenshot

echo '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>DNS Propagation Checker</title>';
...
echo '<h1>DNS Propagation Checker</h1>';
echo '<div class="desc">Check whether recent changes to DNS entries have propagated to DNS servers all over the world. Useful in troubleshooting DNS issues that appear to be isolated to one geographic region. Provides a status report on DNS propagation globally.</div>';
echo '<div class="note">Note: This tool may provide inaccurate results if the domain is using GeoDNS.</div>';
echo '<form class="search-box" method="get">';
echo '<input type="text" name="host" placeholder="Enter hostname" value="' . (isset($_GET['host']) ? htmlspecialchars($_GET['host']) : '') . '" required />';
...
if (isset($_GET['host']) && trim($_GET['host']) !== '') {
    ...
    echo '<div class="tabs">';
    echo '<a href="?host=' . urlencode($host) . '&tab=html" class="tab ' . $html_active . '">HTML</a>';
    echo '<a href="?host=' . urlencode($host) . '&tab=json" class="tab ' . $json_active . '">JSON</a>';
    echo '</div>';
    echo '</div>';
    ...
    echo '</div>';
    // Use backend logic
    $result = get_dns_propagation($host);
    $expected = $result['expected'];
    $servers = $result['servers'];
    echo '<div class="results-header" style="margin-top:0;">DNS Propagation results for <b>' . htmlspecialchars($host) . '</b></div>';
    if ($tab === 'json') {
        $json = [
            "query" => ["tool" => "dns-propagation", "host" => $host],
            "response" => [
        ...
        echo '<table style="margin-top:0;"><tr><th>Location</th><th>IP</th><th>Status</th></tr>';
        foreach ($servers as $row) {
            $status = ($row[1] === $expected) ? '<span class="status-ok">✔</span>' : '<span class="status-error">✖</span>';
            echo '<tr><td>' . htmlspecialchars($row[0]) . '</td><td>' . htmlspecialchars($row[1]) . '</td><td>' . $status . '</td></tr>';
        }
        echo '</table>';
    }
    echo '</div>';
