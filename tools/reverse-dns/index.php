<?php
// Modern UI for Reverse DNS Lookup Tool, inspired by reverse-mx-lookup
require_once __DIR__ . '/backend.php';

function render_header() {
    echo '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Reverse DNS Lookup</title>';
...
function render_main() {
    ...
    echo '<h1>Reverse DNS Lookup</h1>';
    echo '<div class="desc">Find the PTR (reverse DNS) record for an IP address. Useful for network diagnostics and email server validation.</div>';
    ...
    if ($query !== '') {
        ...
        echo '<div class="results-header" style="margin-top:0;">Reverse DNS result for <b>' . htmlspecialchars($query) . '</b></div>';
        if ($tab === 'json') {
            $json = [
                "query" => ["tool" => "reverse-dns", "ip" => $query],
