...
echo '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Spam Database Lookup</title>';
...
echo '<h1>Spam Database Lookup</h1>';
echo '<div class="desc">Check if an IP address or domain is listed in major spam blacklists and reputation databases. Critical for troubleshooting email delivery issues and monitoring organizational IP reputation.</div>';
echo '<form class="search-box" method="get">';
echo '<input type="text" name="query" placeholder="Enter IP or domain (e.g. 1.2.3.4 or example.com)" value="' . (isset($_GET['query']) ? htmlspecialchars($_GET['query']) : '') . '" required />';
...
if (isset($_GET['query']) && trim($_GET['query']) !== '') {
    ...
    echo '<div class="tabs">';
    echo '<a href="?query=' . urlencode($query) . '&tab=html" class="tab ' . $html_active . '">HTML</a>';
    echo '<a href="?query=' . urlencode($query) . '&tab=json" class="tab ' . $json_active . '">JSON</a>';
    echo '</div>';
    ...
    echo '</div>';
    // Use backend logic
    $results = get_spam_database_results($query);
    $count = is_array($results) ? count($results) : 0;
    echo '<div class="results-header" style="margin-top:0;">Spam database results for <b>' . htmlspecialchars($query) . '</b></div>';
    echo '<div class="results-count">Found ' . $count . ' results.</div>';
    if ($tab === 'json') {
        $json = [
            "query" => ["tool" => "spam-database", "query" => $query],
            "response" => [
                "result_count" => strval($count),
                "results" => $results
            ]
        ];
        ...
        echo '<table style="margin-top:0;"><tr><th>Database</th><th>Status</th></tr>';
        if (is_array($results)) {
            foreach ($results as $row) {
                echo '<tr><td>' . htmlspecialchars($row['database']) . '</td><td>' . htmlspecialchars($row['status']) . '</td></tr>';
            }
        }
        echo '</table>';
    }
