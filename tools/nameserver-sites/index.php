<?php
// nameserver-sites/index.php - Find all sites that use a given nameserver
// UI inspired by reverse-mx-lookup/index.php and other tools

echo '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Find all sites that use a given nameserver</title>';
...
echo '<h1>Find all sites that use a given nameserver</h1>';
echo '<div class="desc">Comprehensive tool to list all websites using a specific nameserver. Valuable for infrastructure mapping, competitive analysis, and identifying potential security issues across DNS providers.</div>';
echo '<form class="search-box" method="get">';
echo '<input type="text" name="ns" placeholder="Enter nameserver (e.g. ns1.example.com)" value="' . (isset($_GET['ns']) ? htmlspecialchars($_GET['ns']) : '') . '" required />';
...
if (isset($_GET['ns']) && trim($_GET['ns']) !== '') {
    ...
    echo '<div class="tabs">';
    echo '<a href="?ns=' . urlencode($ns) . '&tab=html" class="tab ' . $html_active . '">HTML</a>';
    echo '<a href="?ns=' . urlencode($ns) . '&tab=json" class="tab ' . $json_active . '">JSON</a>';
    echo '</div>';
    echo '</div>';
    ...
    echo '</div>';
    // Use backend logic
    $domains = get_nameserver_sites($ns);
    $count = count($domains);
    echo '<div class="results-header" style="margin-top:0;">Nameserver results for <b>' . htmlspecialchars($ns) . '</b></div>';
    echo '<div class="results-count">There are ' . $count . ' domains using this nameserver.</div>';
    if ($tab === 'json') {
        $json = [
            "query" => ["tool" => "nameserver-sites", "nameserver" => $ns],
            "response" => [
