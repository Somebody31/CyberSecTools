...
echo '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Traceroute</title>';
...
echo '<h1>Traceroute</h1>';
echo '<div class="desc">Map the complete network path between your location and any destination server. Helps diagnose network latency issues, routing problems, and potential network bottlenecks.</div>';
...
echo '<button type="submit">Trace</button>';
echo '</form>';
if (isset($_GET['host']) && trim($_GET['host']) !== '') {
    ...
    echo '</div>';
    // Use backend logic
    $results = get_traceroute($host);
    $count = is_array($results) ? count($results) : 0;
    echo '<div class="results-header" style="margin-top:0;">Traceroute results for <b>' . htmlspecialchars($host) . '</b></div>';
    echo '<div class="results-count">Found ' . $count . ' hops.</div>';
    if ($tab === 'json') {
        $json = [
            "query" => ["tool" => "traceroute", "host" => $host],
            "response" => [
                "hop_count" => strval($count),
                "hops" => $results
            ]
        ];
        ...
        echo '<table style="margin-top:0;"><tr><th>Hop</th><th>IP</th><th>Latency (ms)</th></tr>';
        if (is_array($results)) {
            foreach ($results as $row) {
                echo '<tr><td>' . htmlspecialchars($row['hop']) . '</td><td>' . htmlspecialchars($row['ip']) . '</td><td>' . htmlspecialchars($row['latency']) . '</td></tr>';
            }
        }
        echo '</table>';
    }
