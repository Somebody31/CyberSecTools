<?php
// Modern UI for ASN Lookup Tool, inspired by reverse-mx-lookup
require_once __DIR__ . '/backend.php';

function render_header() {
    echo '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>ASN Lookup</title>';
...
function render_main() {
    $query = isset($_GET['asn']) ? trim($_GET['asn']) : '';
    $tab = isset($_GET['tab']) ? $_GET['tab'] : 'html';
    $result = $query !== '' ? '' : '';
    echo '<div class="container">';
    echo '<h1>ASN Lookup</h1>';
    echo '<div class="desc">Lookup information about an Autonomous System Number (ASN). Useful for network research and IP ownership analysis.</div>';
    echo '<form class="search-box" method="get">';
    echo '<input type="text" name="asn" placeholder="Enter ASN (e.g. AS15169)" value="' . htmlspecialchars($query) . '" required />';
    ...
    if ($query !== '') {
        ...
        echo '<div class="tabs">';
        echo '<a href="?asn=' . urlencode($tab_query) . '&tab=html" class="tab ' . $html_active . '">HTML</a>';
        echo '<a href="?asn=' . urlencode($tab_query) . '&tab=json" class="tab ' . $json_active . '">JSON</a>';
        echo '</div>';
        ...
        echo '<div class="results-header" style="margin-top:0;">ASN result for <b>' . htmlspecialchars($query) . '</b></div>';
        if ($tab === 'json') {
            $json = [
