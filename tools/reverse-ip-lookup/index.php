<?php
// index.php - Reverse IP Lookup Tool (PHP only, no external CSS/JS/HTML)
// UI inspired by reverse-mx-lookup/index.php and the provided screenshot

echo '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Reverse IP Lookup</title>';
...
echo '<h1>Reverse IP Lookup</h1>';
echo '<div class="desc">Find all domains hosted on a given IP address.</div>';
echo '<form class="search-box" method="get">';
echo '<input type="text" name="q" placeholder="Enter Domain or IP address" value="' . (isset($_GET['q']) ? htmlspecialchars($_GET['q']) : '') . '" required />';
...
if (isset($_GET['q']) && trim($_GET['q']) !== '') {
    ...
    echo '</div>';
    // Use backend logic
    $result = get_reverse_ip_domains($q);
    $ip = $result['ip'];
    $domains = $result['domains'];
    $count = $result['count'];
    echo '<div class="results-header" style="margin-top:0;">Reverse IP results for ' . htmlspecialchars($q) . ' (' . $ip . ')</div>';
    echo '<div class="results-count">There are ' . $count . ' domains hosted on this server.</div>';
    echo '<div class="results-note">The complete listing of these is below:</div>';
    if ($tab === 'json') {
        $json = [
            "query" => ["tool" => "reverse-ip-lookup", "query" => $q],
            "response" => [
                "ip" => $ip,
        ...
        echo '<table style="margin-top:0;"><tr><th style="background:#f3f4f6;font-size:0.95rem;color:#6b7280;font-weight:500;letter-spacing:0.04em;">DOMAIN NAME</th><th style="background:#f3f4f6;font-size:0.95rem;color:#6b7280;font-weight:500;letter-spacing:0.04em;">LAST SEEN</th></tr>';
        foreach ($domains as $row) {
            echo '<tr><td class="domain">' . htmlspecialchars($row[0]) . '</td><td>' . htmlspecialchars($row[1]) . '</td></tr>';
        }
        echo '</table>';
    }
    echo '</div>';
