<?php
// Modern UI for MAC Address Lookup Tool, inspired by reverse-mx-lookup
require_once __DIR__ . '/backend.php';

function render_header() {
    echo '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>MAC Address Lookup</title>';
...
function render_main() {
    $query = isset($_GET['mac']) ? trim($_GET['mac']) : '';
    $tab = isset($_GET['tab']) ? $_GET['tab'] : 'html';
    $result = $query !== '' ? '' : '';
    echo '<div class="container">';
    echo '<h1>MAC Address Lookup</h1>';
    echo '<div class="desc">Lookup the manufacturer or vendor for a MAC address. Useful for network inventory and device identification.</div>';
    echo '<form class="search-box" method="get">';
    echo '<input type="text" name="mac" placeholder="Enter MAC address (e.g. 00:1A:2B:3C:4D:5E)" value="' . htmlspecialchars($query) . '" required />';
    ...
    if ($query !== '') {
        ...
        echo '<div class="tabs">';
        echo '<a href="?mac=' . urlencode($tab_query) . '&tab=html" class="tab ' . $html_active . '">HTML</a>';
        echo '<a href="?mac=' . urlencode($tab_query) . '&tab=json" class="tab ' . $json_active . '">JSON</a>';
        echo '</div>';
        ...
        echo '<div class="results-header" style="margin-top:0;">MAC lookup result for <b>' . htmlspecialchars($query) . '</b></div>';
        if ($tab === 'json') {
            $json = [
                "query" => ["tool" => "mac-lookup", "mac" => $query],
