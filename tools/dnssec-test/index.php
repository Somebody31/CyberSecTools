...
echo '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>DNSSEC Test</title>';
...
echo '<h1>DNSSEC Test</h1>';
echo '<div class="desc">Verify the proper implementation and validity of DNSSEC on any domain. Helps prevent DNS spoofing attacks and ensures the integrity of your domain\'s DNS infrastructure.</div>';
...
if (isset($_GET['domain']) && trim($_GET['domain']) !== '') {
    ...
    echo '</div>';
    // Use backend logic
    $results = get_dnssec_results($domain);
    $count = is_array($results) ? count($results) : 0;
    echo '<div class="results-header" style="margin-top:0;">DNSSEC results for <b>' . htmlspecialchars($domain) . '</b></div>';
    echo '<div class="results-count">Found ' . $count . ' DNSSEC records.</div>';
    if ($tab === 'json') {
        $json = [
